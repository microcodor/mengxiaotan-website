"""
北极星电力网爬虫 - 使用Playwright处理JavaScript渲染
"""
import scrapy
...
class PowerSpider(scrapy.Spider):
    ...
    def start_requests(self):
        """开始请求"""
        urls = [
            'https://news.bjx.com.cn/list/power.html',
    ...
    def parse_article_list(self, response):
        ...
        for link in links:
            ...
            if (10 < len(title) < 100 and 
                ('.shtml' in href or '.html' in href) and
                not any(skip in title for skip in ['首页', '关于', '联系', '更多', '返回', '登录', '注册'])):
    ...
    def parse_article(self, response, meta):
        ...
        # 提取发布时间
        date_text = response.css('.time::text, .date::text, span.time::text').get()
        item['published_at'] = self.parse_date(date_text) if date_text else datetime.now()
        
        # 提取作者
        author = response.css('.author::text, .source::text, span.author::text').get()
        if author:
        ...
        if content_parts:
            ...
            # 尝试提取整个内容区域
            for selector in ['div.content', 'div.article-content', 'div.text']:
