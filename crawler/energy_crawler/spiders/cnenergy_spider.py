"""
中国能源网爬虫 - 使用Playwright处理JavaScript渲染
"""
import scrapy
...
class CnEnergySpider(scrapy.Spider):
    """中国能源网爬虫 - 使用Playwright"""
    name = 'cnenergy'
    ...
    def start_requests(self):
        """开始请求"""
        urls = [
            'https://www.china5e.com/news/news-list-1.html',  # 新闻列表
            'https://www.china5e.com/',  # 首页
        ]
    ...
    def parse_article_list(self, response):
        ...
        for link in links:
            ...
            if (10 < len(title) < 100 and 
                ('.html' in href or '.htm' in href or '/show/' in href) and
                not any(skip in title for skip in ['首页', '关于', '联系', '更多', '返回'])):
                
                if not href.startswith('http'):
                    if href.startswith('/'):
                        href = 'https://www.china5e.com' + href
                    else:
    ...
    def parse_article(self, response, meta):
        ...
        # 提取发布时间
        date_text = response.css('.time::text, .date::text, span.time::text, div.info span::text').get()
        item['published_at'] = self.parse_date(date_text) if date_text else datetime.now()
        
        # 提取作者
        author = response.css('.author::text, .source::text, span.source::text').get()
        if author:
            author = author.strip().replace('来源：', '').replace('作者：', '')
        ...
        if content_parts:
            ...
            for selector in ['div.content', 'div.article', 'div.text', 'div.TRS_Editor']:
