...
class SMMMetalSpider(scrapy.Spider):
    """上海有色金属网爬虫"""
    name = 'smm_metal'
    
    allowed_domains = ['metal.com']
    start_urls = [
        'https://www.metal.com/',
        'https://www.metal.com/news',  # 新闻中心
    ]
    ...
    def parse(self, response):
        ...
        for link in links:
            ...
            if (title and href and 
                ...
                ('/news/' in href or '/article/' in href or '.html' in href)):
                
                title = title.strip()
                
                # 跳过导航链接
                if any(skip in title.lower() for skip in ['home', 'about', 'contact', 'more', 'login', 'register']):
                    continue
                if any(skip in title for skip in ['首页', '关于', '联系', '更多', '返回', '登录', '注册']):
                    continue
                
                # 构建完整URL
                if not href.startswith('http'):
                    if href.startswith('/'):
                        href = 'https://www.metal.com' + href
                    else:
    ...
    def parse_article(self, response):
            ...
            if time_text:
                try:
                    ...
                    if re.match(r'\d{4}-\d{2}-\d{2}', time_text):
                        ...
                        break
                    elif 'T' in time_text:  # ISO格式
                        item['published_at'] = datetime.fromisoformat(time_text.replace('Z', '+00:00'))
                        break
                except:
                    pass
        ...
        if content_parts:
            ...
            # 尝试提取整个内容区域
            for selector in ['div.content', 'div.article', 'div.detail', 'div.news-content', 'article']:
