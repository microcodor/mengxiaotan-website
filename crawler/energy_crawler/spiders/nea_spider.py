"""
国家能源局爬虫 - 使用Playwright处理Vue.js渲染
"""
import scrapy
...
class NeaSpider(scrapy.Spider):
    ...
    def parse_article(self, response, meta):
        ...
        # 提取发布时间
        date_text = response.css('.time::text, .date::text, span.time::text, div.time::text').get()
        ...
        if content_parts:
            ...
            for selector in ['div.TRS_Editor', 'div#TRS_AUTOADD_CONTENT', 'div.content', 'td.b12']:
